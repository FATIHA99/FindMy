...
class DeclarationPerdu extends Controller
{
    ...
    {                                                                                                                                                                                                                                               
        ...
        {
            $objetperdu -> loadData($request -> getBody());
            $objetperdu -> selectobjects($objetperdu->id);  // return one row of the user iformations 
            $objetperdu -> loadData($objetperdu->dataList[0]);

            $currentImage = $objetperdu->dataList[0]['image'];  // ancien image 
            $_SESSION['current_img'] = $currentImage;
            $params=['model' =>$objetperdu];
        ...
        if($request->isPost())
        {
            ...
            return $this->render('updateDecPerdu', [ 'model' => $objetperdu]);  
        }
